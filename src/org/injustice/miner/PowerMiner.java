...
import org.injustice.miner.impl.*;
...
public class PowerMiner extends ActiveScript implements PaintListener, MouseListener, MessageListener {
    ...
    private final ArrayList<StateNode> nodes = new ArrayList<StateNode>();
    ...
    private int addOres;
    private int addExp;
    private final int session = Math.round(System.currentTimeMillis() / 1000);
    private long addon;
    // options
    ...
    public void start() {
        ...
        while (!guiDone) {
            ctx.sleep.sleep(5000);
        }
        Logger.log("GUI complete.");
        Logger.log("Rock selected: " + selectedRock);
        if (banking) {
            Logger.log("Banking enabled.");
            ...
            Logger.log("Location selected: " + master.getLocation());
            Logger.log("Banking at: " + master.getBank());
        }
        Logger.log("Hover next rock: " + rockOption.isHover());
        ...
        Logger.log("Start tile: " + rockOption.getStartTile());
        startExp = ctx.skills.getExperience(Skills.MINING);
        Logger.log("Start exp: " + startExp);
        startLevel = ctx.skills.getRealLevel(Skills.MINING);
        Logger.log("Start level: " + startLevel);
        Logger.log("Rocks loaded: " + getRocks(200));
        Logger.log("Rocks in radius (" + rockOption.getRadius() + "): " + getRocks(rockOption.getRadius()));
    }
    ...
    public void stop() {
        Logger.log("Stopping");
        Logger.log("Updating dynamic sig");
        updateDynamicSig(this.getRuntime() - addon,
                oresMined - addOres,
                (ctx.skills.getExperience(Skills.MINING) - startExp) - addExp,
                session);
        Logger.log("Please post proggies");
