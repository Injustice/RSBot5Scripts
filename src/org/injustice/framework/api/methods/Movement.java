...
public class Movement extends org.powerbot.script.methods.Movement {
    ...
    public boolean walk(Locatable loc) {
        ...
        if (t != null) {
            if (t.getMatrix(ctx).isOnMap()) {
                return dest.equals(t) || ctx.players.local().getLocation().equals(t) || ctx.mouse.click(t.getMatrix(ctx).getMapPoint(), true);
            } else {
                if (dest == null
    ...
    public boolean walkToArea(Area a) {
        return inArea(a) || walk(a.getCentralTile().randomize(5, 5));
    }
